...
题目插图可选上传，也可使用静态安全 SVG 或可编辑函数／坐标图，预览与 PDF 共用渲染。AI 无法可靠复刻时提出上传要求，用户需补图或明确核对已有图足够；原始题图和含批改的图片不自动当作练习配图。检索页可按科目、教材、单本书的任意目录子树限定范围，混合／关键词／连续原文／语义方式集中在展开选项中。
