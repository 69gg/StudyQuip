...
- 上下文预算默认留空，表示不设置应用侧完整请求上限；填写后按该值检查包含思考内容和工具结果的完整请求，此字段不会发送给服务商。教材仍使用独立的材料组织预算（默认 24000，可通过 `STUDYQUIP_CONTEXT_TOKENS` 调整）分块，不把整本书塞进请求。留空不会取消服务商自身的上下文限制。
- 任务失败或中断后，有保存进度时可在任务页或教材详情点击“继续处理”，选择立即或预约续接。复用原任务、已识别页面、已完成正文及工具结果，不从头整理；待校对页仍需人工处理。输入已修改或来源已删除时会说明不能续接的原因，旧检查点保留。
