...
1. 在设置中维护科目，分别配置教材图片、教材文本、题目图片、题目文本及向量嵌入模型。填入服务商 Base URL、API Key、实际模型 ID 和 API 协议；不同用途可以使用同一模型。同用途有多个配置时，当前使用最早配置，删除前请检查运行任务。
...
5. 录入一道错题，填写或由 AI 整理题干、题型、选项、原作答，上传可选参考解析。“AI 整理题目”会将题干和选项中含义明确的公式规范为 LaTeX，支持已有文本与图片识别；不改题意、数值或条件。有歧义的已有字段保留原文并提示核对，原内容保存在版本历史中。题干或选项有变化后，请在预览中检查并重新确认答案。正确答案只能由用户填写或从参考资料提取，**必须由用户确认**。备注始终由用户编辑。
