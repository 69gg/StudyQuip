...
- 上下文预算也可留空，默认留空；这是应用内部的输入预算，不会作为 API 参数发送。留空继承应用统一预算（默认 24000，可通过 `STUDYQUIP_CONTEXT_TOKENS` 调整），填写后以模型配置与应用统一预算中的较小值生效。留空不表示无限上下文，教材仍按受限单元处理。
- 可设置采样参数、超时、重试、工具轮数和扩展 JSON；冲突字段明确报错。
