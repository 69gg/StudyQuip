...
失败、取消或等待人工处理的教材可从任务页和教材详情“继续处理”。沿用原任务、页面状态和操作回执；已识别页直接复用，已完成页和单元跳过，未完成工具链携带完整思考项及工具结果续接，遇到坏页仍暂停。每个新阶段保存初始输入快照；该页草稿版本的分块预算也写入检查点，避免设置变化移动已完成单元的边界。旧检查点缺少这些新增字段时按原流程补建，不要求迁移或删除旧内容。
...
上述预算用于材料组织。模型“上下文预算”留空时不限制完整请求长度；默认材料分块值不能用于拒绝包含思考项与工具结果的续接请求。只有显式填写模型上限时才检查完整请求，超限不裁剪思考或未完成协议项。
