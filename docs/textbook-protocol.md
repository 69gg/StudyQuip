...
模型结果中的数组允许单键 `item` 包装或标准 JSON 数组字符串的明确解码，元素类型与业务约束仍完整校验。继续旧格式失败任务时，若配置兼容且最后一次独立提交能通过当前校验，复用保存的结果进入操作组提交，不再生成同一页；保留原始参数、错误反馈和用量。含额外键、缺失字段或其他无法明确转换的结果继续走模型重试，不能靠丢弃操作通过校验。
...
新修订单元明确区分 `current_page.id`（原页来源）、目录 ID 和已有正文块 ID。草稿已在 `current_page.text`，只新增内容且材料齐备时直接提交；只有缺少依据或要修改已有块时才读取，不要求为新页寻找尚未建立的正文块。已有块修改前读取最新版本的规则不变。
