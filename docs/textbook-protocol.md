...
混合检索组合全文关键词、全文向量、目录标题／概述及目录向量路由、子树补充、概念关系和近邻块。范围在服务端校验；目录候选不能排除全局直接命中。RRF 仅是相对排序分数，不能在界面解释成准确率或置信百分比。返回 `book_title/path/score/channels`；`path` 不含教材页码。生成题目讲解时交给题目文本模型的仍是当前有效正文。
