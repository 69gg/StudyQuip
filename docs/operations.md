...
| CONTEXT_TOKENS / OUTPUT_TOKENS | 24000 / 4096 | 材料分块／初始上下文预算与输出预留；不限制完整工具续接请求 |
...
失败或已取消的任务不会因启动自动恢复。任务页或教材详情出现“继续处理”时，可选择立即或预约，从原任务检查点接续；已识别页面和已提交正文不会重新调用模型。若提示待校对，先校正、重新识别或跳过对应页面。修改输入、删除来源或存在另一互斥任务时，界面显示阻止原因；不用清库来重试。此前因完整请求被默认 24000 预算拦截的任务，在模型上下文预算留空、更新代码并由用户重启应用后，可以通过这个入口续接。
