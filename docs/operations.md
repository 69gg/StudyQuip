...
正式整理长时间停在同一页时，先看请求状态和错误原因：每本书按页顺序提交，只有当前页全部单元通过校验并入库，已整理页数才增加。读取目录／正文、等待模型、网络重试与格式修复都可能发生在同一页内。请求目前等待完整响应，不提供逐 token 进度；“等待模型返回”的秒数不是服务商实际推理时间，无法据此区分排队和生成。较高思考强度、很长的请求超时及历史错误上下文都会放大等待；增加并发上限不会并行提交同书相邻页。不要把进度条未变化误当作已提交内容丢失。
...
本次使用已有 JSON 记录与检查点，不需要新增 Alembic 迁移。旧题目缺少 parts/materials/rendered_figures 时按空列表处理；原科目、题目与教材不删除。用户下次启动新版本 Web/worker 时补齐缺少的默认科目并复用已有同名 ID，重命名后的预设不会在以后启动时重新出现旧名。本次开发验证只用临时数据库和 HTTP 路由夹具，不会替用户运行这些启动步骤。
